...
function setupIpcHandlers(mainWindow, overlayWindow) {
  ...
  ipcMain.on('apply-code', async (event, code) => {
  ...
  // Removed: console.log('IPC handlers setup complete');   console.log('IPC handlers setup complete');
}
module.exports = {
  setupIpcHandlers,
};
